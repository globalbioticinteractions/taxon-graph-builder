...
$(TAXON_CACHE).update:
	...
	cat $(BUILD_DIR)/term_unresolved_twice_corrected.tsv.gz | gunzip | $(NOMER) append --properties=$(NOMER_PROPERTIES_CORRECTED) globi | grep -P "\t(FBC:FB|FBC:SLB:|IRMNG:)" | gzip >> $(BUILD_DIR)/term_resolved.tsv.gz
