...
$(TAXON_CACHE).update:
	...
	cat $(BUILD_DIR)/names_resolved.tsv.gz | gunzip | $(NOMER) append --properties=$(NOMER_PROPERTIES_ID_ONLY) wikidata | grep -v NONE | gzip >> $(BUILD_DIR)/names_resolved_wikidata_only.tsv.gz
	cat $(BUILD_DIR)/names_resolved_wikidata_only.tsv.gz >> $(BUILD_DIR)/names_resolved.tsv.gz
