...
TAXONOMIES:=itis gbif indexfungorum discoverlife ncbi col pbdb tpt mdd batnames worms eol wfo
...
$(TAXON_CACHE):
	...
	cat $(BUILD_DIR)/norm_ranks_tmp.tsv | $(NOMER) replace --properties=$(NOMER_PROPERTIES_ID2NAME) globi-taxon-rank | sed 's/null//g' > $(BUILD_DIR)/norm_ranks.tsv
	...
	cat $(BUILD_DIR)/norm_path_ranks_tmp.tsv | $(NOMER) replace --properties=$(NOMER_PROPERTIES_ID2NAME) globi-taxon-rank | sed 's/null//g' > $(BUILD_DIR)/norm_path_ranks.tsv
