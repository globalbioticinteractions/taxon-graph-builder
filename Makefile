...
$(TAXON_CACHE).update:
	...
	cat $(BUILD_DIR)/term_resolved.tsv.gz | gunzip | grep -v "NONE" | grep -P "(SAME_AS|SYNONYM_OF|HAS_ACCEPTED_NAME_OF|COMMON_NAME_OF)" | cut -f6,7,9-13,15 | sed 's/\$/\t/g' | gzip > $(BUILD_DIR)/term_match.tsv.gz
